...
public class SelectionFinger : SkeletalFinger {
	
	public float filtering = 0.5f;
	public GameObject currentSelection;		//tagged selectable
	...
	protected void RaycastIndexSelection() {
		if (fingerType == Finger.FingerType.TYPE_INDEX){
			Vector3 startPos = Camera.main.transform.position;
			Vector3 endPos = GetTipPosition();
			...
			if (results.Length > 0){
				for( int i=0; i<results.Length; i++ ){
					...
					if ( go.tag == "Selectable" && go != currentSelection){
						currentSelection = go;
					}
				}
