...
//            float pitch = CrossPlatformInputManager.GetAxis("Mouse Y");
			...
			float roll = gameObject.GetComponent<LeapFly>().rollVal;
			float pitch = gameObject.GetComponent<LeapFly>().pitchVal;


//            m_AirBrakes = CrossPlatformInputManager.GetButton("Fire1");
			bool m_AirBrakes = gameObject.GetComponent<LeapFly>().makingFist;



//            m_Yaw = CrossPlatformInputManager.GetAxis("Horizontal");
			float m_Yaw = gameObject.GetComponent<LeapFly>().yawVal;
			//            m_Throttle = CrossPlatformInputManager.GetAxis("Vertical");

			bool withTwoHands = gameObject.GetComponent<LeapFly>().twoHands;

			float m_Throttle;
			if (withTwoHands && !m_AirBrakes){
				m_Throttle = 1;
			}
			else{
				m_Throttle = -1;
			}
//			float m_Throttle = m_AirBrakes ? -1 : 1;
